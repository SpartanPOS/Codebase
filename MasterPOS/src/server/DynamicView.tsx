...
async function getViewComponents() {
  const viewComponents: { [key: string]: () => Promise<any> } = {};
 ...
 */
export async function getView({viewName}: {viewName: string}): Promise<React.ComponentType<any>> {
...
export default async function Page({viewName}: DynamicViewProps): Promise<React.JSX.Element> {
  ...
  const View = await getView({viewName: effectiveViewName});
